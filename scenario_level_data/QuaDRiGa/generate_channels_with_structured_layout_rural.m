...
%% User Parameters
v_min_outdoor = 70;                             %minimal outdoor velocity of users in km/h
v_max_outdoor = 100;                             %maximal outdoor velocity of users in km/h
...
min_dist_to_BS = 100;                    % minimal distance of the users to the BS
max_dist_to_BS = 4000;                   % maximal distance of the users to the BS (multiple of 100)
...
for i_process = 1:rep_factor 
    ...
    % base station
    l.tx_position(3) = 50;  % 25m base station height
    ...
    % set scenario
    indoor_rx = l.set_scenario('3GPP_38.901_RMa_LOS',1:no_of_UEs_outdoor,[],0.0);

    % plot the scenario
    %if i_process == 1
    %    [B1, B2, B3, B4] = compute_sector_boundaries(min_dist_to_BS, max_dist_to_BS, sector_anglespread);
    %    plot_scenario_rural(Mat(1:min(400,length(Mat)),:), B1, B2, B3, B4);
    %end
...
%pre-processing
H_all_rural = squeeze(H_all);
ofdm_channel_real = real(H_all_rural);
ofdm_channel_imag = imag(H_all_rural);

% save data
num_samples_char = char(string(no_of_UEs_perScenario));

path = join(['../ofdm_quadriga_rural_', num_samples_char]);
