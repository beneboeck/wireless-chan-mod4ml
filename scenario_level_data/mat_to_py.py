import h5py
import numpy as np
import argparse

def main():

    parser = argparse.ArgumentParser()

    # Path to your .mat file
    parser.add_argument('-n_samples', '--n_samples', type=int)
    parser.add_argument('-ds', '--dataset', type=str) # 'rural', 'urban'
    args = parser.parse_args()

    path = './'+ 'ofdm_quadriga_' + args.dataset + '_' + str(args.n_samples) + '.mat'

    # Open with h5py
    with h5py.File(path, "r") as f:
        # Check what keys are inside
        #print(list(f.keys()))
        # Load datasets (MATLAB stores them as HDF5 datasets)
        H_all_real = np.array(f['ofdm_channel_real'])
        H_all_imag = np.array(f['ofdm_channel_imag'])

    # save complex valued data + normalize (shuffling is not necessary as it was already in the QuaDRiGa script)
    H_all = H_all_real + 1j * H_all_imag
    H_all = np.transpose(np.squeeze(np.array(H_all)),(2,1,0))
    shape_H = np.prod(H_all.shape[1:])
    H_energy = np.mean(np.sum(np.abs(H_all)**2,axis=(1,2)))
    H_all = H_all * np.sqrt(shape_H / H_energy)

    path_save = './'+ 'ofdm_quadriga_' + args.dataset + '_' + str(args.n_samples) + '.npy'

    np.save(path_save, H_all)

if __name__ == '__main__':
    main()
